...
export default function Home() {
  ...
  useEffect(() => {
    ...
    function parallax(e) {
      document.querySelectorAll('.object').forEach(function(move) {
        ...
        let x = (e.clientX * moveValue) / 800;  
        let y = (e.clientY * moveValue) / 800;

        moveValue === '6' ? y *= -8 : y;
        moveValue === '10' ? y *= -4 : y;
        moveValue === '-16' ? y *= 4 : y;
         
        move.style.transition = 'all .1s 0s ease-in-out';
        move.style.transform = `translateX(${x}px) translateY(${y}px)`;
      });
    };

    parallaxBox.addEventListener('mouseleave', parallaxReset);

    function parallaxReset(e) {
      document.querySelectorAll('.object').forEach(function(move) {
        move.style.transform = `translateX(0px) translateY(0px)`;
        move.style.transition = 'all 1.4s 0s ease-in-out';
      });
    };
  });
  ...
  return(
    <Background>
      <Container id="home">
        <div>
          <a href="https://www.linkedin.com/in/xandebarros/">
            <svg viewBox="0 0 128 128">
              <path d="M116 3H12a8.91 8.91 0 00-9 8.8v104.42a8.91 8.91 0 009 8.78h104a8.93 8.93 0 009-8.81V11.77A8.93 8.93 0 00116 3zM39.17 107H21.06V48.73h18.11zm-9-66.21a10.5 10.5 0 1110.49-10.5 10.5 10.5 0 01-10.54 10.48zM107 107H88.89V78.65c0-6.75-.12-15.44-9.41-15.44s-10.87 7.36-10.87 15V107H50.53V48.73h17.36v8h.24c2.42-4.58 8.32-9.41 17.13-9.41C103.6 47.28 107 59.35 107 75z"></path>
            </svg>
          </a>
          <a href="https://github.com/XandeBarros">
            <svg viewBox="0 0 128 128">
        ...
        <Parallax id='parallax-box'>
          <img className='object' data-value="18" src={cssLogo}></img>
          <img className='object' data-value="6" src={htmlLogo}></img>
          <img className='object' data-value="-5" src={javascriptLogo}></img>
          <img className='object' data-value="10" src={nodejsLogo}></img>
          <img className='object' data-value="-16" src={reactLogo}></img>
        </Parallax>
      </Container>
    </Background>
  );
